...
- Survey missions ask one question at a time (progress bar, auto-advance on multiple-choice, explicit "Lanjut" for free-text) rather than showing every question on one screen. A mission with an optional `estimasi_menit` set additionally renders as a standalone teaser card (badge, big reward, time estimate, full-width CTA) that only reveals the question flow after the user taps in — missions without it keep the plain in-list row.
