...
- Survey missions ask one question at a time (progress bar, auto-advance on multiple-choice, explicit "Lanjut" for free-text) rather than showing every question on one screen.
- Every mission — self-report, link, or survey — renders as its own standalone card (type badge, big reward, optional `estimasi_menit` time estimate with a clock icon, full-width teal CTA) rather than a shared list row. The CTA and body swap per mission type: self-report missions get "Tandai Selesai" that completes immediately; link missions get the CTA label (or a default) that opens the link; survey missions get "Mulai Survei" that reveals the question flow in place (badge + reward + time estimate only, no title/description text). Underlying completion mechanics (self-report, link+return-to-tab, survey submit) are unchanged — only the presentation is unified.
- The Misi tab ends with a "Blog & Info Cuan" section: link-out cards (thumbnail, category badge, title, "Lanjutkan" link) pointing to external channels/articles, defined in a small `lib/blog-cards.ts` array so new cards (e.g. once the Secangkir Cerita blog has posts) are a one-line addition, not a new component.
